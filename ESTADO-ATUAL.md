...
>   - **📋 INVENTÁRIO COMPLETO (um a um) — `decisoes/inventario-agentes-vs-fabrica.md`:** Marcos pediu mapear CADA agente das 5 fontes contra a fábrica (não só os 15 óbvios) → feito: **~650 itens, ~70 OURO, ~190 úteis**. Tem **contagem-mestra** + **KIT-OURO recomendado por etapa da linha** (briefing→copy→arte→gate→distribuição→relatório→produtos→jurídico = exatamente o que baixar/montar 1º). Descobertas: aios-core→aiox-core (squads em aiox-squads; não existe etl/creator-squad); ❓ playwright/cypress = possível motor do render HTML→JPG.
